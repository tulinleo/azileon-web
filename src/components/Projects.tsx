...
export default function Projects() {
  ...
  const labels: Record<Kind, string> = { all: t('filterAll'), hardware: t('hwSectionLabel'), web: t('webSectionLabel') }
  const count = (k: Kind) => (k === 'all' ? all.length : all.filter((p) => p.kind === k).length)
  ...
  return (
    <Panel id="projects">
      <div className="fade-in flex flex-wrap justify-between items-end gap-6 mb-10">
        ...
        <div className="relative grid grid-cols-3 gap-1 bg-page p-1 rounded-btn" role="group" aria-label={t('projectsTitle')}>
          <span
            aria-hidden="true"
            ...
            style={{ transform: `translateX(calc(${KINDS.indexOf(kind)} * (100% + 4px)))` }}
          />
          {KINDS.map((k) => (
            <button
              key={k}
      ...
      >
        {shown.map((p) => (
          <Link key={p.key} to={p.to} className={`group flex flex-col overflow-hidden rounded-tile bg-paper border border-line text-ink no-underline hover:text-ink ${LIFT}`}>
            <div className="aspect-[16/10] bg-page overflow-hidden">
              {p.image && (
                <img src={p.image} alt={p.title} loading="lazy" className="w-full h-full object-cover object-top block transition-transform duration-700 ease-soft group-hover:scale-105" />
              )}
            </div>
