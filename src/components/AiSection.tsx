  ...
  return (
    <button
      ...
      className={`text-left rounded-[18px] px-[22px] py-5 flex gap-4 items-start cursor-pointer border text-paper transition-colors duration-300 ${
        open
          ? "bg-dark-3 border-dark-line"
          : "bg-transparent border-dark-2 hover:border-[#4A4642]"
      }`}
    >
      ...
      <span className="flex flex-col gap-1.5 flex-1 min-w-0">
        ...
        <span
          className={`grid transition-[grid-template-rows,opacity] duration-[450ms] ease-soft ${open ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}
        >
          <span className="overflow-hidden min-h-0">
            <span className="block text-sm leading-[1.6] text-cream-2 pt-0.5">
              {text}
            </span>
          </span>
        </span>
    ...
    <section id="ai" className="grid grid-cols-1 lg:grid-cols-2 gap-bento">
      <div className="fade-in bg-ink text-paper rounded-card p-[clamp(28px,4.5vw,56px)] flex flex-col gap-8">
        <SectionHeader
          ...
          title={t("aiTitle")}
          sub={t("aiSub")}
          className="[&>h2]:mb-0 [&>p:last-child]:max-w-[46ch]"
        />
      ...
      <div className="fade-in stagger-1 bg-surface rounded-card overflow-hidden flex flex-col">
        <div className="px-[clamp(12px,1.4vw,16px)] pt-[clamp(12px,1.4vw,16px)]">
          <a
            ...
            className="group lift lift-sm block"
          >
            <span className="lift-body block rounded-inner overflow-hidden border border-line bg-page">
              <img
        ...
        <div className="p-[clamp(24px,3vw,36px)] flex flex-col gap-[18px]">
          ...
          <div>
            <h3 className="font-heading text-[26px] font-medium tracking-[-0.02em] mb-2">
              {t("aiLiveTitle")}
            </h3>
          ...
          <Button
            href={BATTERY_DISPATCHER_URL}
            arrow="external"
            className="self-start !px-6 !py-3.5"
          >
            {t("aiLiveCta")}
          </Button>
        </div>
