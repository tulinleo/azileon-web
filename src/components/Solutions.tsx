      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,220px),1fr))] gap-3.5">
        {solutions.map((s, i) => (
          <div key={s.slug} className={`fade-in stagger-${Math.min(i + 1, 6)}`}>
            <Link
              to={`/solutions/${s.slug}`}
              className="group lift block h-full text-ink no-underline hover:text-ink"
            >
              <div className="lift-body h-full flex flex-col overflow-hidden rounded-tile bg-paper border border-line">
                ...
                <div className="p-[22px] flex flex-col flex-1">
                  <p className="text-[11px] uppercase tracking-[0.06em] text-accent-hover font-semibold mb-2">
                    {t(s.keys.family)}
                  </p>
                  <h3 className="font-heading text-[21px] font-medium tracking-[-0.01em] mb-2">
                    {t(s.keys.title)}
                  </h3>
                  <p className="text-sm leading-[1.55] text-ink-2 mb-4">
                    {t(s.keys.card)}
                  </p>
                  <ul className="m-0 p-0 list-none flex flex-col gap-[7px] mb-5">
                    {s.features.slice(0, 3).map((f) => (
                      <li
                        key={f}
                        className="flex gap-2 text-[13px] text-ink-2 leading-[1.4]"
                      >
                        <Check
                          size={13}
                          weight="bold"
                  ...
                  <span className="mt-auto inline-flex items-center gap-1.5 text-sm font-semibold text-accent">
                    ...
                    <ArrowRight
                      size={14}
                      weight="bold"
                      className="transition-transform duration-300 ease-soft group-hover:translate-x-[3px]"
                    />
                  </span>
                </div>
              </div>
